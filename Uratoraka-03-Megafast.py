...
def reward_function(params):
    ...
    # penalize the car if the speed is less than 2 m/s
    if speed < 2:
        return 0.001 #return the smallest number possible as a penalty for the agent
    elif (speed >= 2.0 and speed < 2.5):
        speedReward += 1 # reward additional 1 point if the speed is between 2.0 m/s and 2.49 m/s
    elif (speed >= 2.5 and speed < 3.5):
        speedReward += 5 # reward additional 5 points if the speed is between 2.5 m/s and 3.49 m/s
    ...
    if directionDiff > 180:
        directionDiff = 360 - directionDiff #take the direction difference to subtract from 360 if the direction difference is larger than 180
